...
class TestWithdraw(unittest.TestCase):
    ...
    def test_withdraw(self, case_info):
        ...
        if "*amount*" in data:
            user_info = self.db.query("SELECT * FROM member WHERE id = {}".format(self.member_id))
            new_amount = round(user_info["leave_amount"]/10)
            print(new_amount)
            data = data.replace("*amount*",str(new_amount))
            print(data)
        ...
        try:
            ...
            if expected["code"] == 0:
                ...
                a = Decimal(str(before_amount))
                b = Decimal((eval(data)["amount"]))
                c = Decimal(str(after_amount))
                # self.assertTrue(
                #     Decimal(str(before_amount)) - Decimal((data["amount"])) == Decimal(str(after_amount)))
            self.result = "PASS"
